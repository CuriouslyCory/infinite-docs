...
- `interaction` is in the **directional de-dupe key** (ADR-0010/0027), so an interaction
  _edit_ can collide → must return `ConflictError` (a label edit never can).
...
1. **Interaction-edit surface → a SEPARATE `updateEdgeInteraction` mutation**, not an
   ...
   one place that mirrors `connectNodes`. _(The Architect favored extending `updateEdge`;
   decided against for the collision-separation reason above.)_
...
2. **Descend-to-real → Option A: `router.push(/p/[slug]/n/[realEndpointId])`** (the far
   ...
   (route to the canvas _containing_ the real node so it renders as a real node) would need
...
misread (ADR-0031:80-96). Affordance copy is **"Go to {title}"** — _not_ "Descend"/"Open":
...
- **ADR-0027**: one-line "Realized in #65 (`~/lib/connection-direction.ts`)" under
  Consequences **plus a short amendment** recording interaction-_edit_ semantics
...
- **In-passing stale-comment fixes** (files already being edited): the ADR-0023 citation in
  ...
  type) need, not #65 (the picker is an _edit_, not a draw).
...
- **Assumption (verified):** `canConnect` needs **no** interaction arm in #65 — #65 only
  _draws_ `ASSOCIATION`s and _edits_ interaction via `updateEdgeInteraction` (not through
