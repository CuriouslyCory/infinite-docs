...
### Working trace
The **unsaved, client-side set of trace points** a user is currently assembling — the in-progress selection before any named **Trace** is saved. It is a **per-browser working set persisted to `localStorage`, keyed by Project**, so it survives reload but is local to one browser and never written to the server in this form (saving a working trace as a named, shared **Trace** is a later slice). Reaching two or more trace points is what unlocks the **Trace view**'s graph. The word is **working trace** — never "draft trace", "selection", or "pending trace"; it is the *working* set, distinct from a saved **Trace**. *(Realized now as a `localStorage`-backed, per-Project client store feeding the **Component-detail panel** checkbox, the canvas trace-point indicator, and the **Trace view** working-set manager.)*

### Trace view
The route that opens a **Trace** — reached from the always-enabled header **Trace** button, riding the Project **capability slug** so any slug-holder (owner or **viewer**) can open it (ADR-0002). With two or more **trace points** it renders the cross-layer on-path subgraph (a later slice); below that threshold it renders the **working-set manager** — the current **working trace** listed with per-point remove and a clear-all, plus the "add 2+ points to see the graph" empty state. The word is **Trace view** — never "trace page", "trace panel", or "trace canvas" (it is not an editable **Canvas**). *(Realized now as the working-set manager / empty state at `/p/[slug]/trace`; the cross-layer derivation and nested render are a later slice.)*
