...
"endpoint" (that names the route), or "query". A fourth read resource — **`trace`** (`architecture://trace/{traceId}`, #60) — reads one **saved Trace** as deterministic markdown of its cross-layer on-path subgraph; unlike the project-scoped three it is addressed by internal `traceId` and backed by a dedicated owner-gated service, but shares the same token gate, single-401, and non-disclosing not-found. *(Realized now via #18; the trace resource via #60. See ADR-0017, ADR-0022.)*
...
entity is deferred, additive future work.)*
